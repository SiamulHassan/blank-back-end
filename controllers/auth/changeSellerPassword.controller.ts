...
const changeSellerPassword = async (
	...
	try {
		...
		const data = (await Admin.findById(id)) as any;
		...
		// data.password = await bcrypt.hash(password, salt); // already handled in pre save mw
		data.password = password; // already handled in pre save mw
